...
namespace cv
{
    ...
    {
        template <typename T>
        static inline constexpr T saturate_cast(T value) noexcept
        {
    ...
    namespace traits
    {
        ...
        static constexpr bool compare_kernel()
        {
        ...
        static constexpr bool is_kernels_convergent()
        {
        ...
        template <typename... Kernels>
        static constexpr size_t cal_kernels_product()
        {
            size_t product = 1;
    ...
    public:
        ...
        static inline constexpr void operator()(size_t index, Args &&...args) noexcept
        {
    ...
    namespace kernels
    {
        ...
        template <typename ArtihmeticType = float>
        class RGB2HSVPIL final : public Kernel<RGB2HSVPIL<ArtihmeticType>>
        {
        public:
        ...
        template <size_t HRangeMax = 180>
        class RGB2HSVCV final : public Kernel<RGB2HSVCV<HRangeMax>>
        {
        public:
        ...
        template <size_t HRangeMax = 180>
        class RGB2HSVCVNeonAAarch64 final : public Kernel<RGB2HSVCVNeonAAarch64<HRangeMax>>
        {
        public:
        ...
        private:
            ...
            template <size_t N>
            static inline constexpr void v_shr(int32x4_t &store, const int32x4_t &val) noexcept
            {
        ...
        template <size_t HRangeMax = 180>
        class RGB2HSVCVAVX2 final : public Kernel<RGB2HSVCVAVX2<HRangeMax>>
        {
        public:
        ...
        private:
            ...
            template <size_t N>
            static inline constexpr void v_shr_i32x8(__m256i &store, const __m256i &val) noexcept
            {
    ...
    namespace backends
    {
        ...
        public:
            ...
            static inline constexpr size_t operator()(const size_t start, const size_t size, Args &&...args)
            {
                ...
                const size_t chunk_size = concurrency * seq_data_parallel;
                std::atomic<size_t> unprocessed = 0;
                std::forward_list<std::thread> threads;
                const size_t dispatcher_size = size / chunk_size;
                for (size_t i = 0; (i < dispatcher_size) & (i < concurrency); ++i)
                {
                ...
                if (const size_t remain = size % chunk_size; remain > 0)
                {
    ...
    namespace tasks
    {
        ...
        static inline constexpr bool cvtColor(std::span<const uint8_t> in, std::span<uint8_t> out) noexcept
        {
            ...
            return details::cvtColor<BackendType, KernelTypes...>(std::move(in), std::move(out), std::make_index_sequence<Channels>{});
        }
